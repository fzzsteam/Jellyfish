...
- 主应用（Web + API）和 Celery Worker 各自对应独立的 SAE 应用
...
### Celery Worker 独立 SAE 应用

复用同一镜像，通过 SAE 应用的启动命令覆盖来运行 celery worker，与主应用独立扩缩容。
...
| 文件 | 说明 |
|---|---|
| `deploy/docker/combined.Dockerfile` | 多阶段构建：Node 构建前端 → Python 后端 + dist/ |
...
7. 部署主应用到 SAE（`SAE_PROD_APP_ID`）
8. 部署 Worker 到 SAE（`SAE_WORKER_PROD_APP_ID`）
...
4. 安装 aliyun CLI
5. 构建并推送镜像（tag: `test`）
6. 部署主应用到 SAE（`SAE_TEST_APP_ID`）
7. 部署 Worker 到 SAE（`SAE_WORKER_TEST_APP_ID`）
...
| Secret | 说明 |
|---|---|
| `SAE_PROD_APP_ID` | 生产环境主应用 SAE App ID |
| `SAE_WORKER_PROD_APP_ID` | 生产环境 Worker SAE App ID |
| `SAE_TEST_APP_ID` | 测试环境主应用 SAE App ID |
| `SAE_WORKER_TEST_APP_ID` | 测试环境 Worker SAE App ID |
...
| 启动命令 | 默认（`uv run uvicorn app.main:app --host 0.0.0.0 --port 8000`） |
| 监听端口 | 8000 |
...
|---|---|
| 镜像 | 同主应用（同 tag） |
| 启动命令覆盖 | `uv run celery -A app.core.celery_app:celery_app worker -l info` |
