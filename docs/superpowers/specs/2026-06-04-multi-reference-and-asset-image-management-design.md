...
- 支持批量上传本地图片：上传后的图片进入对应资产或镜头图片候选池，用户可选择设为当前图。
...
在资产编辑页当前图片区域或候选池区域增加批量上传入口。推荐使用拖拽上传与文件选择并存：
...
- 前端逐个调用现有上传接口。
- 上传成功后，把返回的 `file_id` 关联到目标图片槽位候选池。
- 若需要更好的批量错误返回和事务语义，再新增批量上传接口。

如果新增候选关联接口，建议保持简单：

- `POST /api/v1/studio/{entity_type}/{entity_id}/images/{image_id}/candidates`
- 请求体包含一个或多个 `file_id`。
- 返回候选列表或新增候选摘要。

具体路由命名可以按现有 Studio API 风格调整，但前端必须使用 OpenAPI generated client。

### 采用规则

- 批量上传图片默认进入候选池，不自动覆盖当前图。
- 如果当前图片槽位为空，可以提供“上传后自动设为当前图”的勾选项，默认关闭或仅在空槽位时启用。
- 用户显式点击“设为当前图”后，再更新当前图片槽位。
...
1. 当前图非空时，新生成图片是否完全不覆盖当前图？推荐：不覆盖，只进候选池。
2. 空槽位时，第一张生成图是否自动设为当前图？推荐：自动设为当前图。
3. 批量上传图片是否默认只进候选池？推荐：只进候选池。
4. 多参考图模式命名是否使用 `multi_reference`？推荐：使用该枚举值，语义清晰且兼容旧模式。
5. 候选池是否新增独立公开接口？推荐：新增语义明确的候选接口，避免前端长期依赖任务链接细节。
