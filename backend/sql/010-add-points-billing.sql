...
-- ============================================================================
SET @has_col = (SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'generation_tasks' AND COLUMN_NAME = 'billing_id');
SET @sql = IF(@has_col = 0,
...
SET @has_idx = (SELECT COUNT(*) FROM information_schema.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'generation_tasks' AND INDEX_NAME = 'ix_generation_tasks_billing_id');
...
INSERT INTO user_points (user_id, balance, frozen, created_at, updated_at)
...
-- billing_id 索引
SET @has_idx = (SELECT COUNT(*) FROM information_schema.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'point_transactions' AND INDEX_NAME = 'ix_point_transactions_billing_id');
SET @sql = IF(@has_idx = 0,
  "ALTER TABLE point_transactions ADD INDEX ix_point_transactions_billing_id (billing_id)",
  'SELECT 1');
PREPARE s FROM @sql; EXECUTE s; DEALLOCATE PREPARE s;

-- (user_id, created_at) 复合索引（按时间倒序查用户流水的常见场景，左前缀同时覆盖纯 user_id 查询）
SET @has_idx = (SELECT COUNT(*) FROM information_schema.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'point_transactions' AND INDEX_NAME = 'ix_point_transactions_user_id_created_at');
SET @sql = IF(@has_idx = 0,
...
SET @has_uq = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'point_transactions' AND CONSTRAINT_NAME = 'uq_point_transactions_billing_type');
SET @sql = IF(@has_uq = 0,
