...
async def list_tasks(
    ...
    recent_seconds: int | None = Query(None, ge=0, le=86400, description="最近结束任务的时间窗口（秒）；不传表示不过滤"),
    ...
    return paginated_response(
        [
            TaskListItemRead(
                ...
                error=item.error,
                error_trace=item.error_trace or None if current_user.is_admin else None,
...
async def get_task_status(
    ...
    return success_response(
        TaskStatusRead(
            ...
            error=view.error,
            error_trace=view.error_trace or None if current_user.is_admin else None,
...
async def get_task_result(
    ...
    return success_response(
        TaskResultRead(
            ...
            result=rec.result,
            error=rec.error,
            error_trace=rec.error_trace or None if current_user.is_admin else None,
